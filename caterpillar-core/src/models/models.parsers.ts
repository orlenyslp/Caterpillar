...
import { ControlFlowInfo, ModelInfo, ParameterInfo } from "./definitions";
    ...
    // So here I fix that as a temporal solution.
        ...
        for (let controlFlowInfo of globalControlFlowInfo) {
            ...
            if (!controlFlowInfo.isEmbedded) {
                ...
                var multiinstanceActivities = [], callActivities = [], nonInterruptingEvents = [];
                    ...
                    if ((node.documentation && node.documentation[0].text && node.documentation[0].text.length > 0)) {
                        ...
                        if(is(node, "bpmn:ServiceTask")) {
                            ...
                            for (let i = 0; i < toIterate.length; i+= 2)
                                controlFlowInfo.localOracleData.get(node.id).push(new ParameterInfo(toIterate[i], toIterate[i + 1]));
                        } else {
