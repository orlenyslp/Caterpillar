...
collectControlFlowInfo = (
  ...
  for (let subprocess of proc.flowElements.filter((e) =>
    ...
    if (
      ...
      controlFlowInfo.boundaryEvents = controlFlowInfo.boundaryEvents.concat(
        subprocessControlFlowInfo.boundaryEvents
      );
    }
  }
...
export let parseModel = (modelInfo: ModelInfo) => {
  ...
  return new Promise((resolve, reject) => {
    parseBpmn(modelInfo.bpmn)
      .then((definitions: any) => {
        ...
        for (let controlFlowInfo of globalControlFlowInfo) {
          if (!controlFlowInfo.isEmbedded) {
              ...
              (nodeId: string, index: number) => {
                ...
                if (
                  ...
                  if (is(node, "bpmn:CallActivity"))
                    controlFlowInfo.externalBundles.set(
                      nodeId,
                      node.documentation[0].text
                    );
                  else
            ...
            let codeGenerationInfo = {
              ...
              getProcessCandidatesMaskFrom: (
                ...
                allEvents.forEach((nodeId) => {
                  ...
                  if (
                    evtType === cad.substring(5, cad.length - 15) &&
              ...
              getCatchingEventsFrom: (procId, evtType, evtCode) => {
                ...
                let parent = globalNodeMap.get(procId);
                let eventFound = false;
                let candidates = controlFlowInfo.boundaryEvents.concat(
                  controlFlowInfo.nodeList
                );
                let eventList = [];
              ...
              getContracts2CallArray: (subprocId, candidates) => {
                let res =
                  "[uint(" + globalNodeIndexMap.get(candidates[0]) + ")";
              ...
              deferredChoiceNodeMarking: (nodeId) => {
                ...
                if (event.incoming) {
                  ...
                  if (is(node, "bpmn:EventBasedGateway")) {
                    for (let outgoing of node.outgoing) {
                      bitarray[
                        controlFlowInfo.nodeIndexMap.get(outgoing.targetRef.id)
                      ] = 1;
                    }
                  }
                }
            ...
            let userTaskList = [];
            let parameterInfo: Map<
              string,
              Map<string, Array<ParameterInfo>>
            > = new Map();
            controlFlowInfo.nodeList.forEach((nodeId) => {
              let node = globalNodeMap.get(nodeId);
            ...
            if (controlFlowInfo.catchingMessages.length > 0)
              userTaskList = userTaskList.concat(
                controlFlowInfo.catchingMessages
              );
            ...
            if (userTaskList.length > 0) {
              modelInfo.solidity += workList2solTemplate(
                workListGenerationInfo
              );
            }
            modelInfo.controlFlowInfoMap.set(
              controlFlowInfo.self.id,
              controlFlowInfo
            );
          } else {
            controlFlowInfo.nodeList.forEach((nodeId) =>
      ...
      .catch((err) => {
        print("Error parsing the process model", TypeMessage.error);
        print(err.toString(), TypeMessage.error);
        reject();
      });
  });
};
