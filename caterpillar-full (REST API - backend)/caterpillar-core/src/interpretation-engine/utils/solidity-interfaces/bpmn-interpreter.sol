...
abstract contract RRegistry {
    function registerIData2IflowRelation(address iDataAddr, address iFlowAddr, address policyOpAddr) public virtual;
...
contract BPMNInterpreter {
    ...
    function createInstance(address cFlow) public returns(address) {
        ...
        IFlow(cFlow).addIDataInstance(pCase);
    ...
    function createInstance(uint eInd, address pCase) private returns(address) {
        ...
        address registryAddr = IFlow(cFlow).getRegistryInst();
        address accCtrol = RRegistry(registryAddr).findRuntimePolicy(pCase);
        RRegistry(registryAddr).registerIData2IflowRelation(cCase, cFlow, accCtrol);
    ...
    function throwEvent(address pCase, bytes32 evCode, uint evInfo) private {
        ...
        if(evInfo & 1540 == 1540 || evInfo & 4612 == 4612) {
    ...
    function tryCatchEvent(address pCase, bytes32 evCode, uint evInfo, bool isInstCompl) private {
        ...
        if(runInstCount == 0) 
            ...
            IData(catchCase).setActivityMarking(pState[1] & ~(1 << subProcInd));
        ...
        // If receiving 10- Default, 11- Terminate or 12- Message
            ...
            // No Instances of the sub-process propagating the event and the sub-process isn't an event-sub-process (BIT 12)
                ...
                if(IFlow(cFlow).getAdyElements(subProcInfo).length > 0)
    ...
    function executeElements(address pCase, uint eInd) public {
        ...
        while (i < count) {
            ...
            if(lInfo[2] & 65 == 65) {
                ...
                pState[0] = IData(pCase).getMarking();
                pState[1] = IData(pCase).getStartedActivities();
                if(lInfo[2] & 128 == 128) // If Intermediate event (BIT 7)
                    pState[0] |=  lInfo[1];
            }
